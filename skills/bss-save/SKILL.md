...
description: |
  ...
  触发方式：/bss-save、/存档、「保存这次诊断」「记下来」「这个结论留着」
  Save the current diagnosis state to disk for cross-session recall.
  Trigger: /bss-save, "save this diagnosis", "remember this"
---

# bss-save：诊断存档

你是 bsskill 的状态保存工具。你的工作是：把当前对话里诊断出来的关键结论、用户已经否决的方向、推荐的下一步，写成一个结构化的 markdown 文件存到本地。
...
bsskill 现有的 13 个诊断 skill，每次对话都是冷启动。用户上周诊断出来的结论、否决的方向，今天再来全部丢失。
...
| 命令 | 行为 |
|---|---|
| `/bss-save` | 存当前对话累积出来的诊断状态。标题由你自动从对话内容提取 |
| `/bss-save <title>` | 用户指定标题，比如 `/bss-save 卖什么没想清楚` |
| `/bss-save list` | 列出当前项目下所有存档 |
| `/bss-save list <项目名>` | 列出指定项目的存档 |
| 「保存这次诊断」「记下来」「这个结论留着」「存档」 | 等价于 `/bss-save` |
...
**用户显式指定**：用 `--slug` 参数，例如 `/bss-save --slug my-project 卖什么没想清楚`。
...
> 现在没什么可存的——前面没做过诊断。先用 `/bss-diagnosis` 或别的诊断 skill 走一轮，再来存档。
...
如果用户用 `/bss-save <title>` 显式指定了，用用户的。

### Step 3：拼路径

```
~/.bss/sessions/{slug}/{YYYYMMDD-HHMMSS}-{title-slug}.md
```
...
title: {标题原文}
source_skill: {对话中主要走过的 skill，比如 bss-diagnosis；走了多个就用逗号分隔}
...
```
已存档：~/.bss/sessions/{项目名}/{文件名}
当前项目下共 {N} 份存档。下次输入 `/bss-restore` 接着上次。
```
...
如果用户输入 `/bss-save list` 或 `/bss-save list <项目名>`，不写新文件，只列出已有存档。
...
1. 2026-04-15 14:23 · 卖什么没想清楚 · 进行中 · 来自 bss-diagnosis
2. 2026-04-22 09:10 · 找到了对标但学不来 · 进行中 · 来自 bss-benchmark
3. 2026-05-01 14:23 · 定价拉到 10 倍价差 · 已结论 · 来自 bss-diagnosis
```
...
如果对话里推荐了下一步 skill（比如 bss-diagnosis 末尾推荐了 `/bss-benchmark`），就填上。
如果没有明显推荐，留空。

不要瞎猜——这个字段是给 bss-restore 自动接续用的，错了比空更糟。

---

## 边界情况

- 用户在没装 bsskill 的环境里调用 → 不存在的情况，bsskill 装了你才会被调用
- 用户连续两次 `/bss-save` 想存同一个状态 → 允许，新文件名带时间戳，不会冲突
- 用户的诊断信息特别敏感（比如收入数字、商业秘密）→ 提醒一句：「`~/.bss/` 是本地纯文本，没有加密。如果不想存敏感细节，可以手动改一下我刚写的文件。」
