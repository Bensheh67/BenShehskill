...
description: |
  ...
  触发方式：/bss-resonate、「这个文稿有没有戳中人」「帮我看看能不能发」「这个完播率会不会低」「文稿发出去有没有流量」
  ...
  Trigger: /bss-resonate, "will this resonate", "check my draft", "will people watch this"
---
...
**不做**：
- 预测平台算法表现（完播率、点击率数字）—— 那由 `/bss-hook` 和 `/全平台标题` 处理
- 检查结构是否完整（是否缺少什么部分）—— 那是另一类问题
- 分析已验证的内容为什么能火 —— 那是 `/bss-spread` 做的事
