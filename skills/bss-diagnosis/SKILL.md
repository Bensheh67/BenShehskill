...
description: |
  ...
  触发方式：/bss-diagnosis、/问诊、「帮我看看商业模式」「诊断一下我的业务」「我有个商业问题」
  ...
  Trigger: /bss-diagnosis, "diagnose my business model", "I have a business question"
---

# bss-diagnosis：商业模式诊断
...
→ 告诉用户：**「这不是一个商业问题，这是一个情绪问题。我的业务边界是商业诊断。建议你用 /bss-action（自检）看看，或者找你信任的人聊聊。」**
...
> 你刚才说了「{原话}」。根据我的判断框架，这更可能是心理问题，不是商业问题。建议用 /bss-action（自检）进一步看看。
...
> 这次诊断如果有结论想留下，输入 `/bss-save` 存档。下次回来 `/bss-restore` 直接接着，不用从头讲。
