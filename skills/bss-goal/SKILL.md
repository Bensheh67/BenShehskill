...
description: |
  ...
  触发方式：/bss-goal、/目标、「帮我搞清楚目标」「我想做个人 IP」「我的目标是成为...」「我想变得更...」
  Goal clarification using Wittgenstein's philosophy of language. Audits fuzzy goals into checkable deliverables.
  Trigger: /bss-goal, "help me clarify my goal", "I want to become...", "my goal is..."
---
...
- 如果浮现的是「做不动」而不是「不知道做什么」→ 审计通过，路由到 `/bss-action`
...
- 三问全部一次通过
- 空转词审查：没有空转词
- 处理：直接告诉用户「这个目标已经清楚，不需要审计。现在你要解决的是执行问题还是路径问题？」→ 路由到 `/bss-action` 或 `/bss-content`

**反面 2：完全答不上来三问的目标**

- 原话：「我想找到属于自己的路」
- 三问：用户全部答不上来或答得都是空话
- 处理：不要强行重写。告诉用户：「现在你脑子里还没有目标，只有一团感觉。建议先做其他诊断：用 `/bss-diagnosis` 搞清楚你现在的商业处境，用 `/bss-benchmark` 看看值得模仿谁。目标是在这些之后才能浮现的。」
