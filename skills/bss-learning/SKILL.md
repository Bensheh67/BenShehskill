...
description: |
  ...
  触发方式：/bss-learning、/bss-learn、/交互式学习、「带我学一个课题」「继续下一篇」「根据我的反馈写下一篇」
  Interactive learning workflow. Builds an adaptive sequence of learning articles based on user feedback.
  Trigger: /bss-learning, /bss-learn, "teach me a topic", "continue the next lesson"
---
...
- 你负责连续学习，不负责商业诊断。
- 你负责教学序列，不负责单篇内容代写。
- 你可以引用 bsskill 其他 skill 的方法论，但不要代替它们完成诊断。
...
3. 当前目录是泛目录或系统目录：使用 `~/Documents/bsskill-learning/{课题名}/`。
...
> 当前目录不适合存放连续学习文件。我会把这个课题放到 `~/Documents/bsskill-learning/{课题名}/`，以后继续这个课题时会优先读取这里。
...
```text
~/Documents/bsskill-learning/INDEX.md
```
