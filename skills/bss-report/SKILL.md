...
name: bss-report
description: |
  把多次 bss-save 攒下来的诊断状态合并成一份可交付的 markdown 报告。
  触发方式：/bss-report、/出报告、「打包」「整理一份」「给合伙人看的」
  Generate a deliverable diagnosis report by merging all bss-save snapshots.
  Trigger: /bss-report, "package this up", "make me a report"
---

# bss-report：诊断报告

你是 bsskill 的报告产物工具。你的工作是：把 bss-save 留下的多份存档文件合并成一份可读、可分享、可归档的诊断报告。

**报告不是你从对话里凭空总结。** 你只读 `~/.bss/sessions/{项目名}/` 下的存档文件，按时间顺序合并、去重、分类。这是报告的可信度来源——它是用户已经确认过的状态的合集，不是 AI 二次发挥。
...
报告把累积的存档固化成一份带日期、带版本、带索引的 markdown 文档。这是 bsskill 从「单次工具」升级到「可交付咨询」的产物。
...
| 命令 | 行为 |
|---|---|
| `/bss-report` | 把当前项目下所有存档合并成报告 |
| `/bss-report --since YYYY-MM-DD` | 只合并某日期之后的存档 |
| `/bss-report --slug <项目名>` | 指定项目 |
| `/bss-report --slug <项目名> --since YYYY-MM-DD` | 同时指定 |
| 「出报告」「打包」「整理一份」「给合伙人看的」 | 等价于 `/bss-report` |
...
按项目找 `~/.bss/sessions/{项目名}/*.md`。

- 0 个文件 → 「`{项目名}` 下没有存档，先用 `/bss-save` 存几次诊断结果再来出报告。」
- 1 个文件 → 提示：「`{项目名}` 下只有 1 份存档，单份不需要合并报告。直接看 `~/.bss/sessions/{项目名}/{文件名}` 就行。」并询问「还是要强制出报告吗？」如果用户说要，继续。
- ≥2 个文件 → 直接进入合并
...
```
~/.bss/reports/{项目名}/{YYYYMMDD-HHMMSS}-{项目名}.md
```
...
|---|---|---|---|---|
| 2026-04-15 | 卖什么没想清楚 | 进行中 | bss-diagnosis | `~/.bss/sessions/{项目名}/20260415-...md` |
| ... | ... | ... | ... | ... |
...
报告由 bsskill 自动生成。原始存档见 `~/.bss/sessions/{项目名}/`。如需更新报告，再次运行 `/bss-report`。
```
...
- 存档文件里有用户标的「敏感信息」（比如真实收入、客户名字）→ 报告原样保留。不做脱敏。这是用户自己存进去的，要脱敏在 bss-save 阶段做
- 多份存档之间结论冲突 → 都列出来，新的在前并标注修正关系
- 用户在 `~/.bss/sessions/` 之外手动放了个文件 → 不读。只读 sessions 目录下的标准文件
