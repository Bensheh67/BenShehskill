...
dbskill 由自媒体博主 dontbesilent 创建，全网可搜 `@dontbesilent 聊赚钱`。dontbesilent 累计发布 16152 条推文，dbskill 将其中经过筛选的内容整理为 4176 个知识原子和 27 个 AI Skills。

适用于任何有真实任务需要推进的人。你可以交付问题、材料、目标、选择或持续卡点，dbskill 会根据当前信息调用合适的 Skill。

**最新更新：v2.17.7**

**v2.17.7 更新**：新增 `dbs-skill-cleaner`。它会审查本地 skill 中的广告导流、隐蔽商业意图、任务劫持、可疑外部调用与敏感数据读取，并在确认后隔离问题 skill。
...
dbskill 当前正式发布 27 个 Skill。下面按用户目标列出入口；每个 Skill 的适用时机、输入示例、预期产出与衔接关系见 [完整 Skill 手册](docs/新手入门.md#skill-全目录)。
...
| 扫描、审查和隔离本地有问题的 skill | `/dbs-skill-cleaner` |
...
版本变更见 [GitHub Releases](https://github.com/dontbesilent2025/dbskill/releases)。当前版本：`v2.17.7`。
